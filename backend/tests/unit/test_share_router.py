...
def test_share_token_creation_and_html_render():
    ...
    # Masked translation sweep (scroll hidden behind overlay, not visible to user)
    assert '#translation-overlay' in html_content
    assert 'sweepTranslationBehindOverlay' in html_content
    assert 'ensureTranslationSweep' in html_content
    assert "behavior: 'instant'" in html_content
    assert 'Finalizing translation' not in html_content
    assert 'behavior: \'smooth\'' not in html_content
